...
%DIF DEL /tmp/icipmc2026-v2-orig.tex   Tue Jul 14 17:58:14 2026
%DIF ADD icipmc2026-v2.tex             Tue Jul 14 17:57:56 2026
...
This paper presents a comprehensive pipeline \DIFdelbegin \DIFdel{for digitizing }\DIFdelend \DIFaddbegin \DIFadd{that digitizes }\DIFaddend scanned chemical literature \DIFdelbegin \DIFdel{that goes beyond }\DIFdelend \DIFaddbegin \DIFadd{beyond simple }\DIFaddend text extraction. The main contributions are: (1) the development of a Dual-Stream Retrieval architecture that processes text and chemical graphs in parallel, integrating both modalities into a unified searchable corpus; (2) the implementation of a local RAG mechanism coupled with strict RDKit-based chemical validation to eliminate generative AI hallucinations; and (3) the application of this pipeline to a historically significant dataset--the acetylene chemistry archives of the Siberian chemical school, including works by academicians Favorsky and \DIFdelbegin \DIFdel{Trofimov--demonstrating }\DIFdelend \DIFaddbegin \DIFadd{Trofimov--which demonstrates }\DIFaddend its effectiveness on \DIFdelbegin \DIFdel{real-world }\DIFdelend \DIFaddbegin \DIFadd{realistic }\DIFaddend degraded scans with complex structural content.

\section{Related Work}

Document digitization systems have evolved from rule-based approaches to deep learning pipelines. \DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{For }\DIFaddend page segmentation, the Recursive X-Y Cut algorithm \cite{ref3} and RLSA \cite{ref4} are foundational methods. LayoutParser \cite{ref5} provides a unified deep learning toolkit for detecting document layout elements, while MinerU \cite{ref6} offers end-to-end extraction from complex publications. LayoutLM \cite{ref7} extends this by jointly pretraining text and layout for document understanding.

\DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend Optical Chemical Structure Recognition (OCSR), deep learning has \DIFdelbegin \DIFdel{shifted the paradigm}\DIFdelend \DIFaddbegin \DIFadd{transformed the field}\DIFaddend : MolScribe \cite{ref8} formulates OCSR as image-to-graph generation, directly predicting atoms and bonds. DECIMER \cite{ref9,ref10} uses a sequence-to-sequence approach, generating SELFIES strings from chemical images. Multimodal vision-language models like Qwen3-VL \cite{ref11} and LLaVA \cite{ref12} have been applied to chemical image understanding through few-shot in-context learning. Reaction segmentation is addressed by RxnScribe \cite{ref13}, which identifies reagent, condition, and product regions in reaction schemes.

Retrieval-Augmented Generation (RAG) has recently been introduced \DIFaddbegin \DIFadd{in}\DIFaddend to chemistry to ground LLM responses in factual data \cite{ref14,ref15}. However, most existing OCSR and RAG systems assume clean, standardized input images and struggle with historical scans featuring complex, degraded layouts. Furthermore, the combination of OCSR-extracted molecular graphs with RAG for historical document archives remains largely unexplored. This paper \DIFdelbegin \DIFdel{bridges the }\DIFdelend \DIFaddbegin \DIFadd{addresses this }\DIFaddend gap by adapting these state-of-the-art models for the specific challenges of historical chemical archives within a \DIFdelbegin \DIFdel{fully localized }\DIFdelend \DIFaddbegin \DIFadd{local }\DIFaddend hardware environment.
...
A critical post-OCSR step is algorithmic validation: every generated SMILES string is \DIFdelbegin \DIFdel{passed through }\DIFdelend \DIFaddbegin \DIFadd{processed by }\DIFaddend the RDKit chemoinformatics library. RDKit verifies the chemical feasibility of the graph, checks valency constraints, calculates implicit hydrogens, and canonicalizes the representation. Invalid structures are flagged for manual review or automated re-processing. This step is essential for eliminating the probabilistic ``hallucinations'' inherent to generative transformer models.

\subsection{Integration: RAG Layer}

The outputs from both streams are semantically linked and stored in a dual database architecture. \DIFdelbegin \DIFdel{Hard }\DIFdelend \DIFaddbegin \DIFadd{Structured }\DIFaddend metadata--page numbers, document identifiers, spatial coordinates--is managed via PostgreSQL. Text embeddings and SMILES fingerprints are indexed in a vector database (ChromaDB). This enables a multimodal RAG system where user queries retrieve both the textual description of a synthesis and its corresponding verified chemical structure, providing grounded, citation-tracked answers without reliance on external cloud services.

\section{Methodology}

\subsection{Adaptive Image Preprocessing}

Scanned historical chemical books exhibit highly variable quality: low resolution, noise, compression artifacts, uneven lighting, and page skew \cite{ref16}. Our adaptive preprocessing algorithm eliminates manual parameter tuning. It converts the image to grayscale, applies Otsu thresholding, and estimates median character height via connected component analysis. The image is scaled so that estimated character height reaches 40 pixels. Adaptive binarization uses a window proportional to four character heights. Noise reduction, contrast, and sharpening are \DIFdelbegin \DIFdel{auto-adjusted}\DIFdelend \DIFaddbegin \DIFadd{automatically adjusted}\DIFaddend . The algorithm operates in two modes: \DIFdelbegin \DIFdel{aggressive }\DIFdelend \DIFaddbegin \DIFadd{an aggressive mode }\DIFaddend for text readability \DIFdelbegin \DIFdel{, conservative }\DIFdelend \DIFaddbegin \DIFadd{and a conservative mode }\DIFaddend for chemical structures to preserve bond topology.
...
Chemical structures appear in \DIFaddbegin \DIFadd{both }\DIFaddend detailed and skeletal notation\DIFdelbegin \DIFdel{s}\DIFdelend  \cite{ref17,ref18}. We adopt SMILES \cite{ref19} as the target representation, with canonicalization via RDKit to resolve representational ambiguity \cite{ref20,ref21}. Three OCSR models are employed:
...
The use of few-shot in-context learning for Qwen3-VL in the OCSR task is methodologically \DIFdelbegin \DIFdel{grounded in }\DIFdelend \DIFaddbegin \DIFadd{based on }\DIFaddend the CAMP framework \cite{ref22}, which demonstrated that transformer models can perform molecular property prediction from context pairs in a single forward pass without fine-tuning. Following this principle, Qwen3-VL is provided with a system prompt and several image--SMILES demonstration pairs, adapting to the OCSR task purely through context rather than parameter updates.
...
When this query is processed, the system operates in parallel\DIFdelbegin \DIFdel{. The }\DIFdelend \DIFaddbegin \DIFadd{: the }\DIFaddend Text-Bibliographic stream searches the vector database (ChromaDB) and retrieves the exact historical text chunks describing the Trofimov reaction methodology---specifically the use of the KOH/DMSO catalytic superbasic system. Simultaneously, the Chemoinformatics stream retrieves the corresponding chemical graphs extracted via OCSR, yielding verified SMILES strings for both the intermediates (e.g., zwitterionic structures) and the final condensed products such as 4,5,6,7-tetrahydroindole (\texttt{C1CCC2=C(C1)C=CN2}).

The local LLM (YandexGPT 5 Lite) then \DIFdelbegin \DIFdel{synthesizes }\DIFdelend \DIFaddbegin \DIFadd{integrates }\DIFaddend these two streams. Because the model is strictly grounded in the retrieved context, it generates a comprehensive, hallucination-free response that describes the synthesis pathway while providing the RDKit-validated structural codes, seamlessly integrating text and chemical topology.
...
Table~\ref{tab:ocr} shows that the combined configuration \textit{OCR+Pre+Norm} achieves the highest mean accuracy (0.7693)\DIFdelbegin \DIFdel{, a }\DIFdelend \DIFaddbegin \DIFadd{. This represents an improvement of approximately }\DIFaddend 35 \DIFdelbegin \DIFdel{\% improvement over }\DIFdelend \DIFaddbegin \DIFadd{percentage points over the }\DIFaddend baseline. LLM-based normalization \DIFdelbegin \DIFdel{alone }\DIFdelend \DIFaddbegin \DIFadd{by itself }\DIFaddend (OCR+Norm) also performs well with the lowest standard deviation. Preprocessing alone provides modest gains, while heuristic normalization underperforms LLM-based correction.
...
As shown in Table~\ref{tab:ocsr}, MolScribe \DIFdelbegin \DIFdel{significantly }\DIFdelend \DIFaddbegin \DIFadd{substantially }\DIFaddend outperforms the other models, achieving 18/20 without preprocessing and perfect recognition on skeletal formulas. Preprocessing did not universally improve OCSR quality: MolScribe's accuracy dropped after preprocessing, suggesting its native image handling is better suited to raw scans. DECIMER's total remained unchanged with a distribution shift, while Qwen3-VL showed a marginal improvement. These results confirm that MolScribe is the most reliable OCSR model for our archival corpus and \DIFdelbegin \DIFdel{guided }\DIFdelend \DIFaddbegin \DIFadd{guides }\DIFaddend our architectural choice in the pipeline.
...
First, the quantitative validation of individual pipeline components was conducted on a relatively modest dataset (30 images), reflecting the project's early stage. The observed trends--particularly the \DIFaddbegin \DIFadd{improvement of approximately }\DIFaddend 35 \DIFdelbegin \DIFdel{\% OCR improvement }\DIFdelend \DIFaddbegin \DIFadd{percentage points in OCR accuracy }\DIFaddend with LLM-based normalization and MolScribe's dominance for OCSR--are consistent enough to guide architectural decisions but should be confirmed on larger, more diverse collections.
...
Third, the current heuristic page segmentation assumes a full-width horizontal block layout\DIFdelbegin \DIFdel{. While appropriate for the monographs in our initial corpus, this approach struggles with }\DIFdelend \DIFaddbegin \DIFadd{, limiting its applicability to }\DIFaddend multi-column journal layouts\DIFdelbegin \DIFdel{and pages featuring side notes}\DIFdelend \DIFaddbegin \DIFadd{, side notes, }\DIFaddend or embedded tables. To address this\DIFdelbegin \DIFdel{limitation, our immediate project roadmap includes the integration of advanced object detection models. Specifically, we are currently }\DIFdelend \DIFaddbegin \DIFadd{, we are }\DIFaddend compiling an annotated dataset of historical multi-column \DIFdelbegin \DIFdel{chemical }\DIFdelend journals to fine-tune YOLOv8 and Vision Transformer (ViT) \DIFdelbegin \DIFdel{architectures}\DIFdelend \DIFaddbegin \DIFadd{models}\DIFaddend . This learned \DIFdelbegin \DIFdel{segmentation }\DIFdelend approach will robustly isolate \DIFdelbegin \DIFdel{dense text, complex }\DIFdelend \DIFaddbegin \DIFadd{text, }\DIFaddend tables, and chemical graphs prior to \DIFdelbegin \DIFdel{the OCSRpipeline, significantly improving the system's }\DIFdelend \DIFaddbegin \DIFadd{OCSR, improving }\DIFaddend adaptability to non-standard archival layouts.
...
We presented a sovereign, locally deployable architecture for the digitization of historical chemical literature, integrating adaptive preprocessing, Dual-Stream Retrieval (text and OCSR), and RAG with RDKit validation. By utilizing EasyOCR with LLM-based normalization for text and models such as MolScribe and Qwen3-VL for chemical graph translation, the system converts raw scans into structured, searchable \DIFdelbegin \DIFdel{knowledge }\DIFdelend \DIFaddbegin \DIFadd{information }\DIFaddend without relying on external cloud services or proprietary databases.

Applying this pipeline to the historical archives of the Siberian chemical school--the acetylene chemistry works of academicians Favorsky and Trofimov--demonstrated its capability to extract complex reaction pathways from degraded 20th-century publications. The mandatory RDKit validation step eliminates AI hallucinations at the chemical structure level, ensuring that the RAG database contains only scientifically sound representations. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend This architecture provides a \DIFdelbegin \DIFdel{robust, }\DIFdelend secure foundation for bringing historical experimental chemistry into the modern data-driven era, enabling \DIFdelbegin \DIFdel{researchers to query }\DIFdelend \DIFaddbegin \DIFadd{querying of }\DIFaddend both textual procedures and verified molecular structures from \DIFdelbegin \DIFdel{documents that were previously accessible only through manual archival study}\DIFdelend \DIFaddbegin \DIFadd{previously inaccessible archival documents}\DIFaddend .
