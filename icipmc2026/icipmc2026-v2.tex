...
Chemical structures appear in detailed and skeletal notations \cite{ref17,ref18}. We adopt SMILES \cite{ref19} as the target representation, with canonicalization via RDKit to resolve representational ambiguity \cite{ref20,ref21}. Three OCSR models are employed:
...
The use of few-shot in-context learning for Qwen3-VL in the OCSR task is methodologically grounded in the CAMP framework \cite{ref22}, which demonstrated that transformer models can perform molecular property prediction from context pairs in a single forward pass without fine-tuning. Following this principle, Qwen3-VL is provided with a system prompt and several image--SMILES demonstration pairs, adapting to the OCSR task purely through context rather than parameter updates.
...
Table~\ref{tab:ocr} shows that the combined configuration \textit{OCR+Pre+Norm} achieves the highest mean accuracy (0.7693), a 35\% improvement over baseline. LLM-based normalization alone (OCR+Norm) also performs well with the lowest standard deviation. Preprocessing alone provides modest gains, while heuristic normalization underperforms LLM-based correction.
...
As shown in Table~\ref{tab:ocsr}, MolScribe significantly outperforms the other models, achieving 18/20 without preprocessing and perfect recognition on skeletal formulas. Preprocessing did not universally improve OCSR quality: MolScribe's accuracy dropped after preprocessing, suggesting its native image handling is better suited to raw scans. DECIMER's total remained unchanged with a distribution shift, while Qwen3-VL showed a marginal improvement. These results confirm that MolScribe is the most reliable OCSR model for our archival corpus and guided our architectural choice in the pipeline.
...
First, the quantitative validation of individual pipeline components was conducted on a relatively modest dataset (30 images), reflecting the project's early stage. The observed trends--particularly the 35\% OCR improvement with LLM-based normalization and MolScribe's dominance for OCSR--are consistent enough to guide architectural decisions but should be confirmed on larger, more diverse collections.
...
Third, the current heuristic page segmentation assumes a full-width horizontal block layout. While appropriate for the monographs in our initial corpus, this approach struggles with multi-column journal layouts and pages featuring side notes or embedded tables. To address this limitation, our immediate project roadmap includes the integration of advanced object detection models. Specifically, we are currently compiling an annotated dataset of historical multi-column chemical journals to fine-tune YOLOv8 and Vision Transformer (ViT) architectures. This learned segmentation approach will robustly isolate dense text, complex tables, and chemical graphs prior to the OCSR pipeline, significantly improving the system's adaptability to non-standard archival layouts.
