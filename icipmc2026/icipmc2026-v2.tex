...
% \IEEEauthorblockN{Shimin Zhao$^{2}$}
  \IEEEauthorblockN{Ivan Trofimov$^{1,3}$, Mikhail Alakshin$^{1,3}$, Evgeny Cherkashin$^{1-4}$,
    Alexander Rybak$^{1,3}$, Dmitry Tkachev$^{1,3}$, Shimin Zhao$^{2}$}\\[0.5em]
...
\email{Marks473a@yandex.ru}, \email{dt7120003@gmail.com}
}
...
Third, the heuristic page segmentation assumes a full-width horizontal block layout. While appropriate for the monographs in our corpus, this approach struggles with multi-column journal layouts and pages featuring side notes or embedded tables. Transitioning to a fully learned segmentation model (e.g., LayoutLM or YOLO-based) is a priority for future work.
